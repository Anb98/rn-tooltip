...
export const choosePosition = ({
  ...
  for (const side of priority) {
    if (space[side] - need[side] > space[bestSide] - need[bestSide])
      bestSide = side;
  }
  return bestSide;
};
...
export const computeArrowOffset = ({
  ...
  const arrowCentre =
    minCentre > maxCentre
      ? crossLength / 2
      : clamp(triggerCross - originCross, minCentre, maxCentre);
  const crossOffset = arrowCentre - ARROW_HALF_WIDTH;

  switch (side) {
    case TooltipPosition.TOP:
      return { x: crossOffset, y: box.height - ARROW_OVERLAP };
    case TooltipPosition.BOTTOM:
      return { x: crossOffset, y: -ARROW_LENGTH + ARROW_OVERLAP };
    case TooltipPosition.LEFT:
      return { x: box.width - ARROW_OVERLAP, y: crossOffset };
    case TooltipPosition.RIGHT:
      return { x: -ARROW_LENGTH + ARROW_OVERLAP, y: crossOffset };
  }
};
...
export const layoutTooltip = (input: LayoutInput): TooltipLayout => {
  ...
  const resolvedWidth = clamp(
    resolveWidth(viewport.width, width),
    0,
  ...
  const idealOrigin = computeBoxOrigin({
    side,
    trigger,
    box,
    viewport,
  ...
  const { origin, shift } = clampToViewport({
    origin: offsetOrigin,
    box,
    viewport,
    margin,
  });
  const arrow = computeArrowOffset({
    side,
    origin,
    box,
    trigger,
